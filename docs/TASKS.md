...
Living register of work, organized by delivery phase (**MVP0 → MVP1 → MVP2 → V1**). Check items off as completed. Full details for each item are in [`SPEC.md`](SPEC.md) §9. The active sprint plan is [`SPRINT4_COMBAT_DEPTH.md`](SPRINT4_COMBAT_DEPTH.md). Architecture context is in [`../CLAUDE.md`](../CLAUDE.md).
...
- [~] Full weapon roster (see spec §4.5) — 7 of 11 weapon types implemented (`baby_missile`, `missile`, `heavy_missile`, `dirt_bomb`, `cluster_bomb`, `baby_nuke`, `nuke`), and a finite ammo economy (`{count, unlimited}`) now exists; remaining stubs are `bouncing_betty`, `funky_bomb`, `napalm`, `shield` (Sprint 4 Slices 2–3)
...
- [~] Shields: purchasable, absorbs one hit — being built as Sprint 4 Slice 3 (destructible particle force field + `use_shield` action through the stack)
