...
- Verified progression uses only `verified_match_results` evidence marked `verified_replay_v1`. The client may submit at most six accepted human `{ angle, power }` commitments. It cannot submit CPU actions, identity, seed, config, outcome, XP, level, rank, totals, or rewards. Completion regenerates every CPU turn through the shared bounded 60-probe policy, independently replays the canonical transcript, and atomically stores one idempotent result plus its result-specific prior/current progression snapshots under a per-account transaction lock. Same-evidence retries return that immutable result and snapshot without replay or a mutable aggregate read; conflicting evidence fails closed.
...
- Start accepts no body. Abandon accepts exactly `{ sessionId }` within 128 UTF-8 bytes. Completion accepts exactly `{ sessionId, transcript }` within 1,024 UTF-8 bytes. The streamed body is authoritative: reads have a 2-second deadline and separately bounded 250 ms cancellation attempt, while `Content-Length` is only an early rejection. Oversized, stalled, invalid-Unicode, malformed, extra-key, ownership, expiry, version, and policy failures return generic responses without raw errors, identifiers, credentials, or partial awards.
...
- Contract version 1 starts disabled in migration 016. The checked-in drain tool controls `starts_enabled`, records authoritative `disabled_at` and `last_started_at`, reports `safe_after`, and refuses enablement until the full 30-minute window has passed and no unexpired session remains. Rollout order is additive migration and backward-compatible functions first, then hosted replay proof, strict-version client, production verification, and explicit enablement. Existing completion and abandon remain available while starts are drained. Replay-semantic changes require a new reviewed contract/drain manifest; version numbers are not dynamic code selectors.
