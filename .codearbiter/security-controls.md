...
- Contract version 1 starts disabled in migration 016. The checked-in drain tool controls `starts_enabled`, records authoritative `disabled_at` and `last_started_at`, reports `safe_after`, and refuses enablement until the full 30-minute window has passed and no unexpired session remains. The protected-floor change is the reviewed V2 transition in `scripts/verified-deployment-v2-rollout.mjs`: drain V1 first, apply migration 017 with V2 disabled, deploy all nine V4/V2 Edge consumers, run the non-awarding hosted replay proof, publish the strict V4 client, enable V2 only once its SQL V1-drain guard accepts, then run the real awarding production V2 retry proof. V2 never dynamically selects V1 replay code; active V1 rows are refused before replay/award while completed V1 rows remain immutable receipts. Existing abandonment remains available during the drain.
