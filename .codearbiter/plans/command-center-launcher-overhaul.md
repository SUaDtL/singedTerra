...
| T44 | IN-PROGRESS | Produce the matched all-mode native evidence set and live resize/interaction recording on the sole 4173 listener, run fresh verification and non-Astra review, update draft PR #515, and stop at `G02`. | `.codearbiter/evidence/pr515-preparation-frame/**`; verification outputs; sprint log; PR metadata | All preparation states are directly reviewed at identical wide/intermediate/compact dimensions; the recording demonstrates edit/focus preservation and reachable actions; exact-head CI is green; PR remains draft/open with no deployment. | `AC-44`, `AC-45` obligations | `AC-44`, `AC-45` | T43 |
